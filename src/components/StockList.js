...
const StockList = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      for (const item of apiData.top_gainers) {
        ...
        name !== 'No active record found for the given symbol' && TopGainersWithNames.push({
          ...item,
          name
        });
      }
      ...
      for (const item of apiData.top_losers) {
        const name = await getActiveNameFromCSV(item.ticker);
        name !== 'No active record found for the given symbol' && TopLosersWithNames.push({
          ...item,
          name
        });
      }
              ...
              {currentPageData.map((item, index) => (
                ...
                >
                  <td className='px-4 py-2 text-left'>
                    {item.ticker}-{item.name.split('- Warrants')[0]}
                  </td>
                  ...
                  >
                    ...
                    {item.change_amount >= 0
                      ? (
                        <i className='ml-1 fas fa-arrow-up' />
                        )
                      : (
                        <i className='ml-1 fas fa-arrow-down' />
                        )}
                  </td>
  ...
  return (
    <>
      ...
      <div className='m-4' id='content-section'>

        <HolidayList isOpen={isOpen} setIsOpen={setIsOpen} />
      </div>
