...
const StockCarousel = ({ stockData, date }) => {
  ...
  return (
    <div className='bg-gray-100 p-4 rounded-lg shadow-md '>
      ...
      <Carousel autoPlay interval={3000} infiniteLoop showStatus showThumbs={false}>
        {chunkedStockData.map((chunk, pageIndex) => (
          <div key={pageIndex} className='flex justify-center'>
            {chunk.map((stock, index) => (
              <div
                key={index} className='p-4 rounded-lg shadow-md border border-gray-300 mx-2 bg-white cursor-pointer' onClick={() => {
              ...
              >
                ...
                <p>
                  Change Amount: <span className={stock.change_amount >= 0 ? 'text-green-500' : 'text-red-500'}>
                    ${stock.change_amount} {stock.change_amount >= 0 ? <i className='fas fa-arrow-up ml-1' /> : <i className='fas fa-arrow-down ml-1' />}
                  </span>
                </p>
                <p>
                  Change Percentage: <span className={stock.change_amount >= 0 ? 'text-green-500' : 'text-red-500'}>
                    {stock.change_percentage} {stock.change_amount >= 0 ? <i className='fas fa-arrow-up ml-1' /> : <i className='fas fa-arrow-down ml-1' />}
                  </span>
                </p>
