...
const Header = () => {
  ...
  return (
    <header className="bg-indigo-600 p-4 flex justify-between items-center">
      ...
      {/* Login Button */}
      {user?<button className="text-white hover:text-blue-300 ml-4" onClick={()=>{
        dispatch(clearCart())
        logOut()
        navigate('/')
      ...
      <button className="text-white hover:text-blue-300 ml-4" onClick={()=>navigate('/login')}>Login</button>}

      {/* Watchlist */}
      <div className="flex items-center space-x-2 ml-4">
        <button className="text-white hover:text-blue-300" onClick={()=>navigate('/watchlist')}>
      ...
      {/* Financial News Link */}
      <Link to="/news" className="text-white hover:text-blue-300 ml-4">Financial News</Link>
